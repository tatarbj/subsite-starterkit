...
node('master') {
    wrap([$class: 'AnsiColorBuildWrapper', cxolorMapName: 'xterm']) {
        stage('Init') {
            ...
            sh "docker run --name $BUILD_ID_UNIQUE -p 127.0.0.1:80:80 -v ${env.WORKSPACE}:/web -v/var/jenkins_home/cache:/var/jenkins_home/cache -v /var/jenkins_home/releases:/var/jenkins_home/releases -w /web -d dev-server:latest"
