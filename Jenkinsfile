    ...
    try {
        wrap([$class: 'AnsiColorBuildWrapper', cxolorMapName: 'xterm']) {
            ...
            stage('Test') {
                ...
                sh "docker exec dev-server ./bin/phing install-dev -D'drupal.db.name'='$DB_NAME' -D'drupal.db.user'='root' -D'drupal.db.password'='password' -logger phing.listener.AnsiColorLogger"
