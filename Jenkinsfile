    ...
    try {
        wrap([$class: 'AnsiColorBuildWrapper', cxolorMapName: 'xterm']) {
            ...
            stage('Test') {
                ...
                sh "docker exec php5-6-php-fpm ls -la /var/www/php5-6"
                sh "docker exec php5-6-php-fpm ./bin/phing install-dev -D'drupal.db.name'='$DB_NAME' -D'drupal.db.user'='$DB_USER' -D'drupal.db.password'='$DB_PASS' -D'drupal.db.su'='root' -D 'drupal.db.su.pw'='password' -logger phing.listener.AnsiColorLogger"
