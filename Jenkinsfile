    ...
    try {
        wrap([$class: 'AnsiColorBuildWrapper', cxolorMapName: 'xterm']) {
            ...
            stage('Test') {
                ...
                sh "docker exec -u jenkins dev-server ./bin/phing install-dev -D'drupal.db.host'='localhost' -D'drupal.db.name'='$DB_NAME' -D'drupal.db.user'='root' -D'drupal.db.password'='' -logger phing.listener.AnsiColorLogger"
